...
@section('content')
    @component('common-components.breadcrumb')
        ...
        @slot('li3') {{ translate('edit transaction') }} @endslot
    @endcomponent
    <div class="create_transaction">
        <div class="container">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="order_store" action="{{ route('products.transactions.update', $transaction) }}" method="POST" enctype="multipart/form-data">
                        @method("PATCH")
                        @csrf
                                ...
                                <div class="col-12 branch_col">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2 branch_select" name="branch_id">
                                            ...
                                            <option value="{{ $branch->id }}" @if($transaction->branch_id == $branch->id) selected @endif>{{ translate($branch->name) }}</option>
                                            @endforeach
                                        </select>
                                        @error('branch_id')
                                    ...
                                    <tbody>
                                        @if($transaction->items->count() > 0)
                                            @foreach ($transaction->items as $item)
                                                @php
                                                    $index = $loop->index;
                                                    $variants = App\Models\ProductVariant::where('product_id', $item['product_id'])->get();
                                                @endphp
                                                <tr id="{{ $index }}">
                                                    ...
                                                    <td width="200">
                                                        <select class="form-control select2 products_search" name="products[{{ $index }}][id]">
                                                            ...
                                                            @foreach ($products as $product)
                                                                <option value="{{ $product->id }}" @if($item['product_id'] == $product->id) selected @endif>{{ $product->sku . ' : ' . $product->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td width="200" class="sizes_td">
                                                        ...
                                                        @if($variants->count() > 0)
                                                            <select class="form-control select2 products_search" name="products[{{ $index }}][variant_id]" data-product_id="{{ $item['product_id'] }}">
                                                                <option value="">{{ translate('choose') }}</option>
                                                                @foreach ($variants as $product_variant)
                                                                    <option value="{{ $product_variant->id }}"
                                                                        @if($item['variant_id'] == $product_variant->id) selected @endif data-variant="{{ $product_variant }}">{{ $product_variant->variant }}</option>
                                                                @endforeach
                                                            </select>
                                                    ...
                                                    <td>
                                                        <input type="number" class="form-control amount" name="products[{{ $index }}][amount]" value="{{ $item['qty'] }}">
                                                        @error("products.$index.amount")
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td><textarea class="form-control" name="products[{{ $index }}][notes]">{{ $item['notes'] }}</textarea></td>
                                                </tr>
                                            @endforeach
                                        @endif
                                        @if(old('products'))
...
@section('footerScript')
    <script>
        ...
        @elseif($transaction->items->count() > 0)
            let index = {{ $transaction->items->count() - 1 }};
        @else
            let index = -1;
        @endif
        function tr(index) {
