            ...
            <div class="card-header">
                <div class="d-flex flex-column flex-md-row text-center text-md-right justify-content-between">
                    ...
                    @can('products.statuses.create')
                        <div>
                            <a href="{{ route('products.statuses.create') }}" class="btn btn-primary mb-2">{{ translate('create') }}</a>
                        </div>
                    @endcan
                </div>
                <form action="{{ route('products.statuses.index') }}" method="GET">
                    <div class="row">
                        <div class="col-6">
                            ...
                            <tbody>
                                @foreach ($statuses as $status)
                                    <tr>
                                        ...
                                        <td>
                                            @if($status->accepted)
                                                <div class="badge badge-success">{{ translate('accepted') }}</div>
                                            @endif
                                        </td>
                                        <td>
                                            @if($status->returned)
                                        ...
                                        <td>
                                            <div class="options d-flex">
                                                @can('statuses.edit')
                                                    <a class="btn btn-info mr-1" href="{{ route('products.statuses.edit', $status) }}">
                                                ...
                                                @can('statuses.destroy')
                                                    ...
                                                    'id' => $status->id,
                                                    'route' => route('products.statuses.destroy', $status->id)
                                                    ])
                                                @endcan
                                        </td>
                                    </tr>
